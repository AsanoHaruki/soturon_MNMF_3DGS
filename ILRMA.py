...
class ILRMA:
    def __init__(self, X_FTM, n_basis=2):
        """ initialize ILRMA
    ...
    def update_WHD(self):
        ...
        self.D_FNM = (tmp_D_SFM / torch.sqrt(norm)
                      [..., None]).conj().permute(1, 0, 2)

    def normalize_WHD(self):
        mu_FN = torch.einsum('fnm,fnm->fn',
                             self.D_FNM.conj(), self.D_FNM).real
        self.D_FNM = self.D_FNM / torch.sqrt(mu_FN[..., None])
    ...
    def separate(self, mic_index=0):
        self.Z_FTN = torch.einsum('fmn,ftn->ftm',  self.D_FNM, self.X_FTM)
        self.Z_FTN = torch.einsum('ftn,fmn->ftnm',
                                  self.Z_FTN,
                                  torch.linalg.inv(self.D_FNM))[..., mic_index]

    def update_log_likelihood(self):
        lk_ent = -(self.Z_pow_NFT / self.l_NFT + torch.log(self.l_NFT)).sum()
        DD = self.D_FNM @ self.D_FNM.adjoint()
        logdetDD = torch.log(torch.linalg.det(DD)).sum().real
        lk = self.n_time * logdetDD.to(torch.complex128)
        lk += lk_ent
...
def main():
    ...
    for i in range(n_source):
        ...
        z_log = 10 * torch.log10(ilrma.Z_FTN[..., i].abs()**2).to("cpu")
        ...
        axs[i+1, 2].imshow(x_log - z_log,
                           aspect='auto', origin='lower')
    fig.tight_layout()
    fig.savefig(save_root/"separated_stft_ilrma.png")
    plt.show()

    recon_sound = stft_tool.istft(ilrma.Z_FTN)
    recon_sound = recon_sound.to("cpu")

    for i in range(n_source):
        sf.write(save_root/f"separated_{i}_ilrma.wav",
