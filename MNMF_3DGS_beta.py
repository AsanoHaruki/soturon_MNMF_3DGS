...
def torch_setup():
    torch.set_default_dtype(torch.float32)  #! float32のほうがいいかも
...
class MNMF:
    ...
    def __init__(self, X_FTM, n_source=2, n_basis=8):   #? 音源数を指定
        ...
        self.mic_array_locs = mic_array_locs
...
class MNMF(MNMF):
    def read_ply_to_tensor(self) -> torch.Tensor:
        ...
        """
        ply_locs = []
        for file_path, translation in zip(self.file_paths, self.ply_translate):
    ...
    def set_U_NIQnD_ply(self):
        ply_locs = self.read_ply_to_tensor(self.file_paths, self.ply_translate)
        self.U_NIQnD = []
        for R_N in ply_locs:
            array_locs = torch.tensor(mic_array_locs).to(self.device)  # (D+1, I)
            # R_N を I 次元に拡張
            ...
            if self.D == 3:
                R_mat[:, 2, 2] = 1.0
            # U_IND に回転を適用
                U_IQnD = (R_mat[:, None, :self.D, :self.D] @ U_IQnD[..., None])[..., 0]  # squeezeを使用せず直接取り出す
            self.U_NIQnD.append(U_IQnD)
